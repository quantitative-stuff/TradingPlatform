...
async fn main() -> anyhow::Result<()> {
    println!("🔍 Debug OKX Full Config - Testing the feeder_direct config");

    // Load the exact same config that feeder_direct uses
    let config_path = "config/crypto/okx_config_full.json";
    let config_str = std::fs::read_to_string(config_path)?;
    let config: ExchangeConfig = serde_json::from_str(&config_str)?;

    println!("📋 Loaded OKX Full Config:");
    ...
    if config.connect_config.ws_url.is_empty() {
        println!("❌ ERROR: WebSocket URL is empty!");
        return Err(anyhow::anyhow!("Empty WebSocket URL"));
    }
    ...
    // Build subscription based on config stream_type
    let mut subscription_args = Vec::new();

    for stream in &config.subscribe_data.stream_type {
        for symbol in test_symbols {
