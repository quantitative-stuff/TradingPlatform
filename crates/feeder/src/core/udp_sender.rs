...
use crate::core::network_utils::get_primary_interface_ip;
...
impl UdpSender {
    pub async fn new(bind_addr: &str, target_addr: &str) -> Result<Self> {
        ...
        if addr_without_port.starts_with("239.") || addr_without_port.starts_with("224.") {
            ...
            if let Some(interface) = get_primary_interface_ip() {
                println!("Detected primary network interface: {}", interface);
            } else {
                println!("Using default network interface for multicast");
            }
    ...
    pub async fn send_trade(&self, trade: &TradeData) -> Result<()> {
        ...
        if let Some(validator) = crate::core::get_packet_validator() {
            validator.validate_pre_send(&trade.exchange, &trade.symbol, trade.timestamp, None);
        }
    ...
    pub async fn send_orderbook(&self, orderbook: &OrderBookData) -> Result<()> {
        // Validate BEFORE sending
        if let Some(validator) = crate::core::get_packet_validator() {
            validator.validate_pre_send(&orderbook.exchange, &orderbook.symbol, orderbook.timestamp, None);
        }
...
pub async fn init_global_udp_sender(bind_addr: &str, target_addr: &str) -> Result<()> {
    ...
    GLOBAL_UDP_SENDER.set(Arc::new(sender))
        .map_err(|_| crate::error::Error::Config("Failed to initialize global UDP sender".to_string()))?;
    Ok(())
}
