...
use super::{QuestDBClient, DatabaseWriter};
...
impl DatabaseUDPReceiver {
    ...
    fn start_crypto_trade_receiver(&self) -> tokio::task::JoinHandle<()> {
        ...
        let questdb = self.questdb_client.clone();
                ...
                move |data: &[u8]| {
                    ...
                    Box::pin(async move {
                        if let Ok(trade) = serde_json::from_slice::<TradeData>(&data) {
                            let client = questdb.write().await;
        ...
        tokio::spawn(async move {
            if let Err(e) = Self::receive_loop(
                ...
                move |data: &[u8]| {
                    ...
                    let stats = stats.clone();
                    Box::pin(async move {
                        let mut client = questdb.write().await;
                        if let Err(e) = client.write_metadata(&data).await {
                            error!("Failed to write crypto metadata: {}", e);
                        } else {
            ...
            if let Err(e) = Self::receive_loop(
                ...
                move |data: &[u8]| {
                    ...
                    Box::pin(async move {
                        let mut client = questdb.write().await;
                        if let Err(e) = client.write_metadata(&data).await {
                            error!("Failed to write stock metadata: {}", e);
                        } else {
        ...
        Fut: std::future::Future<Output = ()> + Send,
    {
        ...
        let multicast_addr: Ipv4Addr = multicast_addr.parse()
            .map_err(|e| Error::Connection(format!("Invalid multicast address: {}", e)))?;
