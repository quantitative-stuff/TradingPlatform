                            ...
                            msg = ws_stream.next() => {
                                match msg {
                                    Some(Ok(Message::Text(text))) => {
                                        ...
                                        process_okx_message(&text, symbol_mapper.clone(), &asset_type);
                                    },
...
fn process_okx_message(text: &str, symbol_mapper: Arc<SymbolMapper>, asset_type: &str) {
                ...
                "books5" | "books" | "books50-l2-tbt" => {
                    ...
                    if let Some(books_array) = data.as_array() {
                        ...
                        for book_data in books_array {
                            // Parse bids and asks
                            let bids: Vec<(f64, f64)> = book_data.get("bids")
                                    ...
                                    arr.iter().filter_map(|level| {
                                        if let Some(level_arr) = level.as_array() {
                                            if level_arr.len() >= 2 {
                                                ...
                                                let qty = level_arr[1].as_str()?.parse::<f64>().ok()?;
                                                Some((price, qty))
                                            } else {
                                                None
                                            }
                            ...
                            let asks: Vec<(f64, f64)> = book_data.get("asks")
                                .and_then(|v| v.as_array())
                                .map(|arr| {
                                    arr.iter().filter_map(|level| {
                                        if let Some(level_arr) = level.as_array() {
