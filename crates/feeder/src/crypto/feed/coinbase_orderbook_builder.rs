// Per-symbol orderbook builder for Coinbase following local_orderbook_builder_plan.md
...
pub struct CoinbaseOrderBookBuilder {
    exchange: String,
    symbol: String,
    asset_type: String,
...
impl CoinbaseOrderBookBuilder {
    ...
    /// Process an orderbook update (snapshot or update)
    pub fn process_update(&mut self, update: CoinbaseOrderBookUpdate) {
        match update.message_type.as_str() {
            "snapshot" => {
                debug!("[Coinbase] [{}] Processing snapshot with {} bids, {} asks",
            ...
            "update" => {
                debug!("[Coinbase] [{}] Processing update with {} bid updates, {} ask updates",
                    self.symbol, update.bids.len(), update.asks.len());
                ...
                if let Some(orderbook) = &mut self.orderbook {
                    ...
                    warn!("[Coinbase] [{}] Received update before snapshot, ignoring", self.symbol);
                }
            },
            _ => {
                warn!("[Coinbase] [{}] Unknown message type '{}', ignoring",
    ...
    fn publish_orderbook(&self, orderbook: &OrderBookData) {
        ...
        debug!("[Coinbase] [{}] Published orderbook with {} bids, {} asks",
...
pub fn spawn_orderbook_builder(
    ...
    symbol: String,
    asset_type: String,
) -> mpsc::Sender<CoinbaseOrderBookUpdate> {
    let (tx, mut rx) = mpsc::channel::<CoinbaseOrderBookUpdate>(1000);

    let symbol_clone = symbol.clone();
    tokio::spawn(async move {
        let mut builder = CoinbaseOrderBookBuilder::new(
            exchange,
            symbol,
            asset_type,
        );

        debug!("[Coinbase] Orderbook builder task started for {}", symbol_clone);

        while let Some(update) = rx.recv().await {
            builder.process_update(update);
        }

        debug!("[Coinbase] Orderbook builder task stopped for {}", symbol_clone);
    });
